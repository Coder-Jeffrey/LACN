...
import random
# 生成不重复的113个0到255之间的随机数字列表
random_list = random.sample(range(12), 5)
# 对列表进行从小到大排序
random_list.sort()
print('----------------random number----------------------')
print(random_list)
word_embeddings = torch.nn.Embedding(5, 10)
print()
def classification(epochs, batch_size, learning_rate, weight_decay):
    ...
    for i in jst:
        ...
        for epoch in range(epochs):
            ...
            test_loss, test_prec, test_ndcg, test_metrics,psp_1,psp_3,psp_5 = test_model(test_dataloader, device, model, optimizer, random_list)
