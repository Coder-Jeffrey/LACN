...
class ClassifyModel11585(nn.Module):
    ...
    def classifier(self,optimizer,input_ids, input_mask, token_type_ids, fit_docspace_position, fit_labelspace_position, labels, ground_label, iftrain, labels293,gama):
        if iftrain!=0:
            optimizer.zero_grad()
        ...
        label_output = torch.sigmoid(self.linear1(H_enc))
        temp = label_output
        pred_3956 = self.get_origin_label_yuan(temp, labels293)
        ...
        # p_a5 = preLoss[4][0]
        # print(p_a5)
        # x = np.array(label_output.cpu().detach().numpy())
        # y = np.array(labels.cpu().detach().numpy())
        #
        # print('-------------------------')
        # print(label_output)
    ...
    def forward(self, optimizer, input_ids, input_mask, token_type_ids, fit_docspace_position, fit_labelspace_position,labels, ground_label, iftrain, labels293,gama):
        label_output, loss, label_index = self.classifier(optimizer, input_ids, input_mask, token_type_ids, fit_docspace_position, fit_labelspace_position,labels, ground_label, iftrain, labels293,gama)
        return label_output, loss, label_index
    ...
    def get_origin_label_yuan(self, label_out, ground_labels293):
        ...
        #
        # for i in range(self.batch_size):
        #     for index, (antecedent, consequent) in enumerate(zip(self.antecedents, self.consequents)):
        #         proSum = 0.0
        ...
        #         predicted_prob = proSum*self.confidence[index]
    ...
    def get_origin_label(self, label_out, ground_labels293):
        ...
        temp293 = torch.from_numpy(label293).float()
        raw293_loss = self.loss_func(temp293, ground_labels293)
    ...
    #
    #     ground_label = ground_label.cpu()
    #     gl = ground_label.numpy()
    #     a = self.antecedents
    #     b = self.consequents
    #     c = self.confidence
    #     d = self.merge_id
    #     e = self.frequent_index
    #     m = d[43]
    #     e1 = e[15]
    #     e2 = e[251]
    #     label_out200 = np.where(label_out200 >= 0.5, 1., 0.)
